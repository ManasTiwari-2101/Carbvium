            ...
            <AnimatePresence>
                {isExpanded && (
                    <>
                        ...
                        >
                            <motion.div
                                className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden max-w-7xl w-full my-auto flex flex-col md:flex-row max-h-[95vh] md:max-h-[90vh]"
                                onClick={(e) => e.stopPropagation()}
                            >
                                {/* Left Side - Image & Price (40%) */}
                                <div className="md:w-2/5 flex flex-col bg-gradient-to-br from-gray-100 to-gray-200 p-4 sm:p-6 shrink-0">
                                    {/* Car Image - Reduced to 60% height */}
                                    <div className="flex-1 flex items-center justify-center relative mb-3 sm:mb-4 min-h-0">
                                        <img
                                            src={carImage}
                                            alt={car.name || car.model_name}
                                            className="max-w-full max-h-[120px] sm:max-h-full object-contain"
                                            onError={(e) => {
                                                e.target.src = "https://images.unsplash.com/photo-1552519507-da3b142c6e3d?w=600";
                                            }}
                                        />
                                        {/* Vehicle Type Badge */}
                                        <span className={`absolute top-2 left-2 sm:top-4 sm:left-4 text-xs sm:text-sm font-semibold px-2 sm:px-3 py-1 rounded-full ${getTypeColor(car.type || car.vehicle_type)}`}>
                                    ...
                                    <div className="grid grid-cols-3 gap-1 sm:gap-2 mb-2">
                                        {car.horsepower && (
                                            <div className="bg-white p-2 sm:p-3 rounded-lg sm:rounded-xl shadow-sm text-center">
                                                <p className="text-xs sm:text-sm text-gray-900">Horsepower</p>
                                                <p className="text-sm sm:text-lg font-bold text-gray-800">{car.horsepower} HP</p>
                                            </div>
                                        )}
                                        {car.mileage && (
                                            <div className="bg-white p-2 sm:p-3 rounded-lg sm:rounded-xl shadow-sm text-center">
                                                <p className="text-xs sm:text-sm text-gray-900">Mileage</p>
                                                <p className="text-sm sm:text-lg font-bold text-gray-800">{car.mileage} {(car.type || car.vehicle_type)?.toLowerCase() === "ev" ? "km/ch" : "km/l"}</p>
                                            </div>
                                        )}
                                        {car.category && (
                                            <div className="bg-white p-2 sm:p-3 rounded-lg sm:rounded-xl shadow-sm text-center">
                                                <p className="text-xs sm:text-sm text-gray-900">Category</p>
                                                <p className="text-sm sm:text-lg font-bold text-gray-800">{car.category}</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {(car.price || car.price_inr_lakhs) && (
                                        <div className="bg-white p-3 sm:p-4 rounded-lg sm:rounded-xl shadow-sm">
                                            <p className="text-sm sm:text-l text-gray-900">Price</p>
                                            <p className="text-xl sm:text-2xl font-bold text-gray-800">₹{car.price || car.price_inr_lakhs} Lakhs</p>
                                        </div>
                                    )}
                                </div>

                                {/* Right Side - Details (60%) */}
                                <div className="md:w-3/5 p-4 sm:p-5 md:p-6 flex flex-col space-y-3 sm:space-y-4 overflow-y-auto">
                                    ...
                                    <div className="bg-gradient-to-br from-blue-50 to-purple-50 p-2 sm:p-3 rounded-xl sm:rounded-2xl shadow-md border border-blue-100 [&_*]:outline-none [&_svg]:focus:outline-none">
                                        <h3 className="text-sm sm:text-md font-bold text-gray-700 mb-1 sm:mb-2 text-center">
                                            🌍 CO₂ Lifecycle
                                        </h3>
                                                ...
                                                <ResponsiveContainer width="100%" height={150} className="sm:!h-[190px]">
                                                    <PieChart>
                                                        <Pie
                                                            ...
                                                            className="sm:[&_path]:!outerRadius-[65]"
                                                        >
                                    ...
                                    <div className={`p-2 sm:p-3 rounded-lg sm:rounded-xl text-center font-bold ${getCO2BadgeColor(car.co2 || car.total_lifecycle_co2_kg)}`}>
                                        <p className="text-xs sm:text-sm font-semibold mb-1">Total Lifecycle CO₂</p>
                                    ...
                                    <div className="space-y-2 text-xs">
                                        <h3 className="text-xs sm:text-sm font-bold text-gray-700 border-b pb-1">
                                            📊 Emission Breakdown
                                        </h3>
                                        ...
                                        <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                                            ...
                                            <div className="p-1.5 sm:p-2 bg-blue-50 rounded-lg border border-blue-200">
                                                <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-1">
                                                    ...
                                                    <p className="font-semibold text-gray-700 text-xs sm:text-sm">Manufacturing</p>
                                                </div>
                                            ...
                                            <div className="p-1.5 sm:p-2 bg-orange-50 rounded-lg border border-orange-200">
                                                <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-1">
                                                    <span className="text-sm sm:text-lg">🚗</span>
                                                    <p className="font-semibold text-gray-700 text-xs sm:text-sm">Running</p>
                                                </div>
                                            ...
                                            {/* Battery CO2 */}
                                            <div className="p-1.5 sm:p-2 bg-purple-50 rounded-lg border border-purple-200">
                                                <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-1">
                                                    <span className="text-sm sm:text-lg">🔋</span>
                                                    <p className="font-semibold text-gray-700 text-xs sm:text-sm">Battery</p>
                                                </div>
                                            ...
                                            {car.lifecycle_intensity_kg_per_km && (
                                                <div className="p-1.5 sm:p-2 bg-teal-50 rounded-lg border border-teal-200">
                                                    <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-1">
                                                        <span className="text-sm sm:text-lg">📏</span>
                                                        <p className="font-semibold text-gray-700 text-xs sm:text-sm">CO₂/KM</p>
                                                    </div>
